  ...
  fetch('/api/shops')
    ...
    .then(async data => {
      ...
      if (!itemTemplates || itemTemplates.length === 0) {
        try {
          const items = await fetch('/api/item_templates').then(res => res.json());
          itemTemplates = items;
        } catch (e) {
...
function renderItems(tab) {
  ...
  tab.items.forEach((item, itemIndex) => {
    ...
    card.innerHTML = `
      ...
      ${item.description ? `<div class="item-desc text-xs text-gray-500 mb-1">${item.description}</div>` : ''}
  ...
  loadOptions.then(() => {
    ...
    if (tempIdInputEl && descPreviewEl) {
      function updateDesc() {
        ...
        descPreviewEl.textContent = tpl && tpl.description ? tpl.description : '';
      }
...
function showEditItemDialog(itemIndex) {
  ...
  modal.innerHTML = `
    ...
    ${item.description ? `<div style="margin-bottom:10px;">
    ...
    <div style="margin-bottom:10px;">
      ...
      <input type="checkbox" id="edit-isSell" ${item.isSell !== false ? 'checked' : ''}>
    </div>
    <div style="margin-bottom:10px;">
      <label>Icon Spec (item_spec):</label><br>
      <input type="number" id="edit-iconSpec" style="width:100%;" value="${item.iconSpec || 0}">
    </div>
    <div style="margin-bottom:10px;">
  ...
  modal.querySelector('#save-edit').addEventListener('click', () => {
    ...
    const newIconSpecVal = parseInt(modal.querySelector('#edit-iconSpec').value, 10);
    if (newIsNew !== item.isNew) updates.isNew = newIsNew;
    if (newIsSell !== item.isSell) updates.isSell = newIsSell;
    if (!isNaN(newIconSpecVal) && newIconSpecVal !== item.iconSpec) updates.iconSpec = newIconSpecVal;
  ...
  playersData.forEach(player => {
    ...
    card.innerHTML = `
      <div class="flex items-center">
        <img src="${player.avatarIcon || '/icons/x4/3013.png'}" alt="${player.name}" class="w-12 h-12 rounded-full mr-4">
        <div class="flex-1">
...
function renderGiftcodeForm() {
  ...
  const createGiftcodeOptionRows = (selectedOpts) => {
    return optionTemplates.map(opt => {
      const existing = Array.isArray(selectedOpts) ? selectedOpts.find(o => o.id === opt.id) : undefined;
      const checked = existing ? 'checked' : '';
      const paramVal = existing ? existing.param : '';
      return `
        <div class="giftcode-option-row" style="margin-bottom:6px;">
          <label><input type="checkbox" class="gc-opt-checkbox" value="${opt.id}" ${checked}> ${opt.description}</label>
          <input type="number" class="gc-opt-param" placeholder="param" style="width:60px; margin-left:4px;" ${checked ? '' : 'disabled'} value="${paramVal}">
        </div>
      `;
    }).join('');
  };
  ...
  function addGiftcodeItemRow(containerEl, existingItem) {
    ...
    const itemOptionsHtml = itemTemplates.map(it => `<option value="${it.id}" ${existingItem && existingItem.temp_id == it.id ? 'selected' : ''}>${it.name}</option>`).join('');
    const qtyVal = existingItem && existingItem.quantity ? existingItem.quantity : 1;
    row.innerHTML = `
      <div style="display:flex; flex-wrap:wrap; gap:8px; align-items:flex-end;">
        ...
        <div class="gc-options-container" style="margin-top:6px; max-height:150px; overflow-y:auto; border:1px solid #ddd; padding:4px; width:100%;">
          ${createGiftcodeOptionRows(existingItem && existingItem.options ? existingItem.options : [])}
        </div>
  ...
  form.querySelector('#create-giftcode').addEventListener('click', (ev) => {
    ...
    const countLeftVal = parseInt(form.querySelector('#giftcode-count').value, 10);
    ...
    listEl.querySelectorAll('.gc-item-entry').forEach(row => {
      ...
      row.querySelectorAll('.giftcode-option-row').forEach(optRow => {
        ...
        if (cb.checked) {
          const id = parseInt(cb.value, 10);
          const param = parseInt(optRow.querySelector('.gc-opt-param').value, 10);
          if (!isNaN(id) && !isNaN(param)) {
            opts.push({ id, param });
          }
        }
      ...
      if (!isNaN(tempId)) {
        items.push({ temp_id: tempId, quantity: isNaN(qty) ? 1 : qty, options: opts });
      }
    });
    if (items.length === 0) {
      msgEl.style.color = '#c00';
...
function loadMailForm() {
  ...
  if (itemTemplates && itemTemplates.length > 0 && optionTemplates && optionTemplates.length > 0) {
    renderMailForm();
    return;
  }
  Promise.all([
    itemTemplates && itemTemplates.length > 0 ? Promise.resolve(itemTemplates) : fetch('/api/item_templates').then(res => res.json()),
    optionTemplates && optionTemplates.length > 0 ? Promise.resolve(optionTemplates) : fetch('/api/item_option_templates').then(res => res.json())
  ]).then(([items, options]) => {
    if (!itemTemplates || itemTemplates.length === 0) itemTemplates = items;
    if (!optionTemplates || optionTemplates.length === 0) optionTemplates = options;
    renderMailForm();
  }).catch(err => {
    const container = document.getElementById('mail-form-container');
    if (container) container.innerHTML = '<p>Lỗi tải dữ liệu item/option.</p>';
    console.error(err);
  });
}
...
function renderMailForm() {
    ...
    return optionTemplates.map(opt => {
      ...
      return `
        <div class="mail-option-row" style="margin-bottom:6px;">
          <label><input type="checkbox" class="mail-opt-checkbox" value="${opt.id}" ${checked}> ${opt.description}</label>
          <input type="number" class="mail-opt-param" placeholder="param" style="width:60px; margin-left:4px;" ${checked ? '' : 'disabled'} value="${paramVal}">
        </div>
      `;
    }).join('');
  };
    ...
    row.innerHTML = `
      <div style="display:flex; flex-wrap:wrap; gap:8px; align-items:flex-end;">
        ...
        <div class="mail-options-container" style="margin-top:6px; max-height:150px; overflow-y:auto; border:1px solid #ddd; padding:4px; width:100%;">
          ${createMailOptionRows(existingItem && existingItem.options ? existingItem.options : [])}
        </div>
  ...
  form.querySelector('#send-mail-button').addEventListener('click', (ev) => {
    ...
    const nameVal = (form.querySelector('#mail-player-name').value || '').trim();
    ...
    listEl.querySelectorAll('.mail-item-entry').forEach(row => {
      ...
      row.querySelectorAll('.mail-option-row').forEach(optRow => {
        ...
        if (cb.checked) {
          const id = parseInt(cb.value, 10);
          const param = parseInt(optRow.querySelector('.mail-opt-param').value, 10);
          if (!isNaN(id) && !isNaN(param)) {
            opts.push({ id, param });
          }
        }
...
function showPlayerDetail(player) {
  // Build stats HTML similar to renderPlayers
  let statsHtml = '';
  if (player.stats) {
    if (Array.isArray(player.stats)) {
      statsHtml = '<ul class="list-disc list-inside text-sm text-gray-700">' + player.stats.map((val, idx) => `<li>Chỉ số ${idx}: ${val}</li>`).join('') + '</ul>';
  ...
  modal.innerHTML = `
    <h3 class="mb-2 font-bold text-xl">${player.name}</h3>
    <div class="mb-2 text-sm text-gray-700">Level ${player.level} | Power ${player.power}</div>
    <div class="mb-4 text-sm text-gray-700">HP ${player.hp}/${player.hpBase} | MP ${player.mp}/${player.mpBase}</div>
  ...
  document.body.appendChild(overlay);
  // Close button
  modal.querySelector('#close-player-detail').addEventListener('click', () => {
    document.body.removeChild(overlay);
  });
}
...
function showEditGiftcodeDialog(gfc) {
  ...
  const ensure = (itemTemplates && itemTemplates.length > 0 && optionTemplates && optionTemplates.length > 0)
    ? Promise.resolve()
    : Promise.all([
        fetch('/api/item_templates').then(res => res.json()).then(data => { itemTemplates = data; }),
        fetch('/api/item_option_templates').then(res => res.json()).then(data => { optionTemplates = data; })
      ]);
  ensure.then(() => {
    ...
    modal.innerHTML = `
      ...
      <div style="margin-bottom:10px;">
        <label>Code:</label><br>
        <input type="text" id="edit-gc-code" style="width:100%;" value="${gfc.code}" disabled>
      </div>
      <div style="margin-bottom:10px;">
        <label>Số lần sử dụng (countLeft):</label><br>
        <input type="number" id="edit-gc-count" style="width:100%;" min="0" value="${gfc.countLeft}">
      </div>
      <div style="margin-bottom:10px;">
        <label>Loại giftcode (type):</label><br>
      ...
      <div style="margin-bottom:10px;">
        ...
        <input type="date" id="edit-gc-expired" style="width:100%;" value="${gfc.expired ? new Date(gfc.expired).toISOString().split('T')[0] : ''}">
      </div>
    ...
    if (gfc.items && Array.isArray(gfc.items)) {
      gfc.items.forEach(it => {
        ...
        addGiftcodeItemRow(listEl, { temp_id: it.temp_id, quantity: it.quantity, options: opts });
      });
    }
    // Add item button
    ...
    modal.querySelector('#edit-gc-save').addEventListener('click', () => {
      const countLeftVal = parseInt(modal.querySelector('#edit-gc-count').value, 10);
