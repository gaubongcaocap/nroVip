...
public class SummonDragon {
    ...
    private SummonDragon() {
        ...
        this.update = Thread.startVirtualThread(() -> {
            while (active && !Maintenance.isRunning) {
                try {
                    ...
                    Functions.sleep(1000); // nếu là Thread.sleep thì virtual thread vẫn ổn
                } catch (Exception e) {
    ...
    private boolean checkShenronBall(Player pl) {
        ...
        if (!pl.isActive()) {
            Service.gI().sendThongBao(pl, "Vui lòng kích hoạt để gọi rồng thần");
            return false;
        }
        if (pl.nPoint.power < 150000000L) {
            Service.gI().sendThongBao(pl, "Yêu cầu đạt 150tr sức mạnh mới có thể gọi được rồng thần");
            return false;
        } else if (dragonStar == 1) {
                ...
                switch (this.select) {
                    case 0: // đẹp trai nhất vũ trụ
                        if (InventoryService.gI().getCountEmptyBag(playerSummonShenron) > 0) {
                            ...
                            avtVip.itemOptions.add(new ItemOption(50, Util.nextInt(1, 20)));
                            avtVip.itemOptions.add(new ItemOption(101, Util.nextInt(1, 15)));
                            avtVip.itemOptions.add(new ItemOption(5, Util.nextInt(5, 15)));
                            avtVip.itemOptions.add(new ItemOption(14, Util.nextInt(5, 15)));
                            avtVip.itemOptions.add(new ItemOption(95, Util.nextInt(5, 15)));
                            avtVip.itemOptions.add(new ItemOption(96, Util.nextInt(5, 15)));
                            avtVip.itemOptions.add(new ItemOption(0, Util.nextInt(1, 2000)));
