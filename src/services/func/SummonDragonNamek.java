    ...
    this.update = Thread.startVirtualThread(() -> {
        while (active) {
            try {
                ...
                Functions.sleep(1000); // Có thể dùng Thread.sleep hoặc bất kỳ hàm delay nhẹ nào
            } catch (Exception e) {
                e.printStackTrace(); // hoặc dùng Logger nếu bạn có
            }
        }
    });
                            ...
                            if (playerSummonShenron.clan != null) {
                                playerSummonShenron.clan.members.forEach(m -> {
                                    if (Client.gI().getPlayer(m.id) != null) {
                                        ...
                                        Player p = PDTSqlFetcher.loadById(m.id);
                                        if (p != null) {
                                            Item it = ItemService.gI().createNewItem((short) 16);
                                            it.quantity = 1;
                                            InventoryService.gI().addItemBag(p, it);
