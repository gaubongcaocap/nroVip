  ...
  private void changeMap(Player pl, Zone zoneJoin, int mapId, int zoneId, int x, int y, byte typeSpace) {
    ...
    if (zoneJoin != null) {
        ...
        if (x != -1) {
            ...
            pl.location.x = (zoneJoin.map.mapWidth > 200) ? Util.nextInt(100, zoneJoin.map.mapWidth - 100) : 100;
        }
        pl.location.y = y;
        ...
        if (currMapIsCold != nextMapIsCold && !pl.isBoss && !pl.isBot) {
    ...
    private void sendEffectMeToMap(Player player) {
        ...
        try {
            ...
            if (player.mobMe != null) {
                ...
                msg.writeLongBySoulmate(Util.maxIntValue(player.mobMe.point.gethp()), cn.readInt);// hp mob
                Service.gI().sendMessAnotherNotMeInMap(player, msg);
                msg.cleanup();
            }
            if (player.pet != null && player.pet.mobMe != null) {
                ...
                msg.writeLongBySoulmate(Util.maxIntValue(player.pet.mobMe.point.gethp()), cn.readInt);// hp mob
                Service.gI().sendMessAnotherNotMeInMap(player, msg);
                msg.cleanup();
            }
        } catch (Exception e) {
            ...
            for (Player pl : players) {
                if (!player.equals(pl)) {
                    ...
                    if (pl.mobMe != null) {
                        ...
                        msg.writeLongBySoulmate(Util.maxIntValue(pl.mobMe.point.gethp()),cn.readInt);// hp mob
                        player.sendMessage(msg);
                        msg.cleanup();
                    }
                }
