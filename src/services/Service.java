...
import boss.list.Commeson.NhanBan;
import boss.list.Training.TrainingBoss;
...
//        }
    ...
    public void regisAccount(Session session, Message _msg) {
        try {
            ...
            if (rs.next()) {
                ...
                DBConnecter.executeUpdate("insert into account (username, password, admin , cash, danap) values()", user, pass, 0, 0, 0);
    ...
    public void Send_Info_NV(Player pl) {
        ...
        try {
            ...
            msg.writer().writeInt((int) pl.id);
            msg.writeLongBySoulmate(Util.maxIntValue(pl.nPoint.hp), cn.readInt);
            msg.writer().writeByte(0);//Hiệu ứng Ăn Đậu
            msg.writeLongBySoulmate(Util.maxIntValue(pl.nPoint.hpMax), cn.readInt);
 ...
 public void ClosePanel(Player pl) {
    ...
    public void point(Player player) {
        ...
        if (!player.isPet && !player.isBoss && !player.isNewPet) {
            ...
            try {
                ...
                msg.writer().writeInt((int) player.nPoint.hpMax);
                msg.writer().writeInt((int) player.nPoint.mpMax);
                msg.writer().writeInt((int) player.nPoint.hp);
                msg.writer().writeInt((int) player.nPoint.mp);
                msg.writer().writeByte(player.nPoint.speed);
                msg.writer().writeByte(20);
                msg.writer().writeByte(20);
                msg.writer().writeByte(1);
                msg.writer().writeInt((int) player.nPoint.dame);
    ...
    public void hsChar(Player pl, long hp, long mp) {
        ...
        try {
            ...
            msg.writeLongBySoulmate(Util.maxIntValue(mp), cn.readInt);
...
//                Logger.logException(Service.class, e);
    ...
    public void sendChiSoPetGoc(Player pl) {
        ...
        try {
            ...
            msg.writeLongBySoulmate(Util.maxIntValue(pl.pet.nPoint.hpg), cn.readInt);
            msg.writeLongBySoulmate(Util.maxIntValue(pl.pet.nPoint.mpg), cn.readInt);
            msg.writeLongBySoulmate(Util.maxIntValue(pl.pet.nPoint.dameg), cn.readInt);
    ...
    public void showInfoPet(Player pl) {
        if (pl != null && pl.pet != null) {
            ...
            try {
                ...
                msg.writeLongBySoulmate(Util.maxIntValue(pl.pet.nPoint.hp), cn.readInt); //hp
                msg.writeLongBySoulmate(Util.maxIntValue(pl.pet.nPoint.hpMax), cn.readInt); //hpfull
                msg.writeLongBySoulmate(Util.maxIntValue(pl.pet.nPoint.mp), cn.readInt); //mp
                msg.writeLongBySoulmate(Util.maxIntValue(pl.pet.nPoint.mpMax), cn.readInt); //mpfull
                msg.writeLongBySoulmate(Util.maxIntValue(pl.pet.nPoint.dame), cn.readInt); //damefull
...
//                if (key.equals("NroLegion") && code == 31072002 && token.equals("NguyenDucVuEntertainment")) {
...
//            }
    ...
    public void stealItemFromPlayer(Player thief, Player victim, ItemMap item) {
        if (InventoryService.gI().findItemBag(victim, item.itemTemplate.id) != null && InventoryService.gI().findItemBag(victim, item.itemTemplate.id).quantity >= item.quantity) {
            Service.gI().dropItemMap(thief.zone, item);
            thief.zone.pickItem(thief, item.itemMapId);
            InventoryService.gI().subQuantityItemsBag(victim, InventoryService.gI().findItemBag(victim, item.itemTemplate.id), item.quantity);
    ...
    public void sendMabuAttackSkill(Player player) {
        ...
        try {
            ...
            for (Player plM : player.zone.getNotBosses()) {
                ...
                msg.writeLongBySoulmate(Util.maxIntValue(damage), cn.readInt);
            }
            sendMessAllPlayerInMap(player, msg);
            msg.cleanup();
        } catch (Exception e) {
...
//    //========================CHIEN TRUONG NAMEK========================
//    public void sendPhuBanInfo(Player player, int idMapPaint, String nameTeam1, String nameTeam2, int maxPoint,
...
//    }
//
//    public void sendPhuBanInfo_UpdatePoint(Player player, int pointTeam1, int pointTeam2) {
//        Message msg;
//        try {
//            msg = new Message(20);
//            msg.writer().writeByte(0); // Chien Truong Namek
...
//    }
//
//    public void sendPhuBanInfo_UpdateLife(Player player, int lifeTeam, int lifeTeam2) {
//        Message msg;
//        try {
//            msg = new Message(20);
//            msg.writer().writeByte(0); // Chien Truong Namek
