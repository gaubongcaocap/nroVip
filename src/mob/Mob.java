...
//        }
        ...
        if (player.itemTime.isUseMayDo && (Util.isTrue(1, 100) || (player.isActive() && Util.isTrue(1, 50))) && this.tempId > 57 && this.tempId < 66) {
        ...
        //Set kich hoat
        if (((Util.isTrue(1, 50)) || (Manager.TEST && Util.isTrue(1, 50)) || player.isPl() && Util.isTrue(1, 2500) && MapService.gI().isMapUpSKH(mapid))) {
            short itTemp = (short) ItemService.gI().randTempItemKichHoat(player.gender);
            ItemMap it = new ItemMap(zone, itTemp, 1, x, yEnd, player.id);
            List<Item.ItemOption> ops = ItemService.gI().getListOptionItemShop(itTemp);
            if (!ops.isEmpty()) {
...
//        }
        ...
        //Sao pha le
        if (Util.isTrue(1, 200) || (player.nPoint.isDoSPL && Util.isTrue(5, 100)) || (player.isActive() && Util.isTrue(1, 100))) {
        ...
        //Da nang cap
        if (Util.isTrue(1, 200) || (Util.isTrue(5, 300) && MapService.gI().isMapTuongLai(mapid)) || (player.isActive() && Util.isTrue(5, 300))) {
        ...
        if (MapService.gI().isMapCold(mapid)) {
            ...
            if (Util.isTrue(1, 20000) || (player.isActive() && Util.isTrue(1, 200000)) || (player.isAdmin() && Util.isTrue(10, 50))) {
                ItemMap it = ItemService.gI().randDoTL(this.zone, 1, x, yEnd, player.id);
                list.add(it);
               
               ChatGlobalService.gI().autoChatGlobal(player, "[ Hệ Thống ] " + player.name + " vừa nhặt được " + it.itemTemplate.name + " tại " + this.zone.map.mapName + " khu " + this.zone.zoneId);
            }
            if ((Util.isTrue(1, 500) || (player.isActive() && Util.isTrue(2, 100))) && InventoryService.gI().fullSetThan(player)) {
...
//        if (MapService.gI().isMapTuongLai(mapid) && (Util.isTrue(1, 200000) || Util.isTrue(1, 10000))) {
...
//            ChatGlobalService.gI().sendThongBaoTheGioi(player, "[ Hệ Thống ] " + player.name + " vừa nhặt được " + it.itemTemplate.name + " tại " + this.zone.map.mapName + " khu " + this.zone.zoneId);
//        }
//
//        // Kiểm tra xem bản đồ có phải là "Map Nappa"
//        if (MapService.gI().isMapNappa(mapid) && (Util.isTrue(1, 200000) || Util.isTrue(1, 10000))) {
...
//        }
       ...
       if (Util.isTrue(1, 10000) || (player.isActive() && Util.isTrue(1, 11000))) {
        ...
        // Manh thien su
        if ((Util.isTrue(1, 1000) || (player.isActive() && Util.isTrue(10, 300))) && MapService.gI().isMapHanhTinhThucVat(mapid) && InventoryService.gI().findItemNTK(player)) {
