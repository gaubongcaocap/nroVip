...
public final class WorldMartialArtsTournament implements Runnable {
    ...
    private void init() {
        ...
        Thread.startVirtualThread(this::run); // gọn nhất, an toàn, đúng chức năng
    }
    ...
    public void finish() {
        try {
            thongBao();
            if (plWin != null) {
                if (WorldMartialArtsTournamentManager.gI().round == 1) {
                    TaskService.gI().checkDoneTaskDHVT(plWin);
                }
                if (WorldMartialArtsTournamentManager.gI().round == 2) {
                    TaskService.gI().checkDoneTaskDHVT(plWin);
                    TaskService.gI().checkDoneTaskDHVT(plWin);
                    TaskService.gI().checkDoneTaskDHVT(plWin);
                    TaskService.gI().checkDoneTaskDHVT(plWin);
            ...
            // Các thông báo và xử lý cho người thắng
