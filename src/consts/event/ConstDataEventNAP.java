...
// su kien 1/6
public class ConstDataEventNAP {//Zalo: 0385436233

    public static ConstDataEventNAP gI;

    public static ConstDataEventNAP gI() {//Zalo: 0385436233
    ...
    public static final byte DATE_END = 17;
    public static final byte HOUR_END = 5;
    public static final byte MIN_END = 18;
 ...
 public static boolean isActiveEvent() {
    ...
    if (System.currentTimeMillis() >= startEvent.getTimeInMillis() && System.currentTimeMillis() <= endEvents.getTimeInMillis()) {
        if (isTraoQua && System.currentTimeMillis() + 60000 >= endEvents.getTimeInMillis()) {
            String sql = "SELECT player.id as plid, player.name as name, player.data_task FROM player "
                    + "WHERE player.data_task IS NOT NULL "
                    + "ORDER BY CAST(SUBSTRING_INDEX(data_task, ',', 1) AS UNSIGNED) DESC, "
            ...
            int actualSize = Math.min(AccIdPlayer.size(), 10); // Lấy số lượng người chơi thực tế nếu ít hơn 10
