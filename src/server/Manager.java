...
//                clan.hpClan = rs.getLong("hp");
//                clan.kiClan = rs.getLong("ki");
//                clan.dameClan = rs.getLong("dame");
//                clan.critClan = rs.getInt("crit");
//                clan.defClan = rs.getLong("def");
                CLANS.add(clan);
            }
        }
    ...
    private void loadTasks(Connection con) throws SQLException {
        try (PreparedStatement ps = con.prepareStatement(
                ...
                + "FROM task_main_template JOIN task_sub_template ON task_main_template.id = task_sub_template.task_main_id"); ResultSet rs = ps.executeQuery()) {
            int taskId = -1;
            TaskMain task = null;
            while (rs.next()) {
                int id = rs.getInt("id");
