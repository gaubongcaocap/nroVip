...
public class Nezuko extends Boss {
    ...
    public void reward(Player plKill) {
        ...
        if (itemId == 1802) {
            ChatGlobalService.gI().autoChatGlobal(plKill,  "[ Hệ Thống ] " + plKill.name + " đã tiêu diệt được Nezuko và nhặt được 500k VNĐ");
        }
    ...
    public void doneChatE() {
        ...
        for (Boss boss : this.parentBoss.bossAppearTogether[this.parentBoss.currentLevel]) {
            if ((boss.id == BossID.INOSUKE_PIG || boss.id == BossID.ZENITSU || boss.id == BossID.INOSUKE) && !boss.isDie()) {
                return;
            }
        }
